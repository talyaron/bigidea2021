...
function NavTopBar({role}) {
    ...
    return (
        <div className="topNavContainer">
            {!isLogin? <div className="titleDisplay">
                <img className="NewLogoTopNav" onClick={() => {navigate('/MainPage');}} src={titleDisplay} alt="NBN TLV Logo" />
                <img className="NewLogoTopNav" onClick={() => {navigate('/MainPage');}} src={titleSecondDisplay} alt="NBN TLV Alt Logo" />
							...
							<ul className='menuList'>
								{
									navList.map(function (item) {
										return <li className='ListItemElement' key={item.id} id={item.id}>
											<div
												className='menuItem'
												onClick={() => {
